...
# check for the nodeset (<...>.uanodes) file if present and mount it into the container
if [[ -n "$ACTION_MODEL_ABSPATH" ]]; then
  echo "Custom model used for server: $ACTION_MODEL_ABSPATH"
  DOCKER_ARGS+=" -v ${ACTION_MODEL_ABSPATH}:/usr/src/app/assets/KRITIS3M.Reference.PredefinedNodes.uanodes"
fi

echo docker run -d $DOCKER_ARGS dotnet-server $CONTAINER_ARGS 
docker run -d $DOCKER_ARGS dotnet-server $CONTAINER_ARGS
